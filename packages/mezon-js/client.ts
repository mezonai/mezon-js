...
export class Client {
 ...
 async banGroupUsers(session: Session, groupId: string, ids?: Array<string>): Promise<boolean> {
  ...
  async listChannelUsers(session: Session, channelId: string, clanId: string, state?: number, limit?: number, cursor?: string): Promise<ApiChannelUserList> {
    if (this.autoRefreshSession && session.refresh_token &&
        session.isexpired((Date.now() + this.expiredTimespanMs)/1000)) {
        await this.sessionRefresh(session);
    }

    return this.apiClient.listChannelUsers(session.token, channelId, clanId, limit, state, cursor).then((response: ApiChannelUserList) => {
