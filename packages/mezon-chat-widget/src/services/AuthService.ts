...
import type { AuthResult, AuthState, IAuthService, ISessionService, Unsubscribe, UserInfo } from './interfaces';
...
export class AuthService implements IAuthService {
  ...
  private async getAuthUrl(): Promise<string> {
    ...
    const url = `${this.config.apiBaseUrl.replace(/\/+$/, '')}/${oauthPath.replace(/^\/+/, '')}`;
