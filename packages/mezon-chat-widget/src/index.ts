...
// @ts-ignore
import './MezonChatElement'; // Auto-register custom element
...
export class MezonLightChat {
  ...
  public logout(): void {
    ...
    // Reinitialize widget to show login view
    if (this.widget && this.container) {
      this.widget.destroy();
      this.widget = new ChatWidget(this.config, () => this.handleLoginRequest(), undefined, () => this.launcher?.toggle(), this.config.iconHeader);
...
// Export for global usage - Make it the default export for IIFE
