...
export class MezonClient implements Client {
  ...
  constructor(
    ...
    this.apiClient = new MezonApi(apiKey, timeout, basePath);
  ...
  async reactionMessage(
    ...
    const msgReaction = await this.socket.writeMessageReaction(
      id,
      clan_id,
      channel_id,
      mode,
  ...
  async updateChatMessage(
    ...
    if (messageLength > 4096 * 2) throw "Message exceeds allowed characters";
    const msgUpdated = await this.socket.updateChatMessage(
      clan_id,
      channel_id,
      mode,
      is_public,
  ...
  async authenticate() {
    return this.apiClient
      .authenticateMezon(this.apiKey, "", { token: this.apiKey })
  ...
  async logout(session: Session) {
    ...
    return this.apiClient.sessionLogout(session.token, request).then((response) => {
  ...
  async createChannelDesc(request: ApiCreateChannelDescRequest): Promise<void | ApiChannelDescription> {
    ...
    return (await this.apiClient.createChannelDesc(session.token, request)) as ApiChannelDescription;
  }
  ...
  async registerStreamingChannel(request: ApiRegisterStreamingChannelRequest) {
    ...
    return this.apiClient.registerStreamingChannel(session.token, request);
  }
}
